...
test('replace string', () => {
  const updated = jsu(original, "foo", (_) => "sommick");
  expect(original.foo).toEqual("fooValue");
  expect(updated.foo).toEqual("sommick");
})

test('replace whole array', () => {
  const updated = jsu(original, "bars", (_) => [1,2,3]);
  expect(original.bars.length).toBe(2);
  expect(updated.bars).toEqual([1,2,3]);
})

test("replace value in array", () => {
  const updated = jsu(original, "bars[0]", (_) => {return {test: true}});
...
test('replace nested value in array', () => {
  const updated = jsu(original, "bars[0].ork", (_) => "newOrk1Value");
